...
class SeqOutTest extends AnyFunSuite {
  ...
  def simRandomBase(dut: SeqOut, bufferDepth: Int, simTask:(RxDriver, RxDriver, PsnGenerator, Int)=>Unit): Unit ={
    dut.clockDomain.forkStimulus(2)
    fork {
      while (true) {
        dut.io.tx.ready #= !dut.io.tx.ready.toBoolean
    ...
    val (rxOtherDriver, rxReadRespDriver, psnSetter) = setInputDrivers(dut)
    ...
    for (_ <- 0 to 100) {
      ...
      psnSetter.set(psn)
      psnGen.setPsn(psn)
      simTask(rxOtherDriver, rxReadRespDriver, psnGen, packetNum)
      waitTargetPsn(dut, (psn until psn + packetNum).toList)
    }
  }

  def simRxOtherRespRandom(dut: SeqOut, bufferDepth: Int): Unit = {
    simRandomBase(dut, bufferDepth, (rxOtherDriver, rxReadRespDriver, psnGen, packetNum)=>{
  ...
  def simSeqOutRandom(dut: SeqOut, bufferDepth:Int): Unit ={
    simRandomBase(dut, bufferDepth, (rxOtherDriver, rxReadRespDriver, psnGen, packetNum)=>{
      ...
      while(nPacketGenerated<packetNum){
        ...
        rxReadRespDriver.push(psnGen.genNextOrder(m))
